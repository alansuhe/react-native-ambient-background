...
> ⚠️ **重要提示**：本项目使用 pnpm 作为包管理器。在开始之前，请确保已安装 pnpm：
>
> ```bash
> npm install -g pnpm
> ```
>
> 请勿使用 npm 或 yarn，因为它们无法正确解析 workspace 配置。
...
> Since the project relies on pnpm workspaces, please make sure you have [pnpm](https://pnpm.io/) installed.
...
```sh
Running "AmbientBackgroundExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
