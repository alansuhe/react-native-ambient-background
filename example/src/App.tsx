...
export default function App() {
  const [animationVariant, setAnimationVariant] = useState<AnimationStyle>('ripple');
      ...
      />
      {
        AnimationStyles.map(variant => {
          ...
          return (
            ...
            >
              <Text style={{ color: 'white', fontSize: isSelected ? 32 : 16 }}>{variant}</Text>
            </TouchableOpacity>
          )
        })
      }

      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 64 }}>
        {
          Colors.map(color => (
            <TouchableOpacity
              key={color}
              onPress={() => setMainColor(color)}
              style={[styles.colorPicker, { backgroundColor: color }, color === mainColor && styles.bordered]}
            />
          ))
        }
